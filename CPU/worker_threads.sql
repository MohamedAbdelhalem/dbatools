...
go
select 
[max worker threads], [Current worker thread],
...
from (
select [max worker threads] = case when c.value = 0 then case 
...
end end else c.value end,cwt.[Current worker thread], wq.[avg work queue count],
case when c.value = 0 then 'auto' else 'manual' end [configuration type], wv.win_version + case 
...
cross apply (SELECT SUM(current_workers_count) as [Current worker thread] FROM sys.dm_os_schedulers) cwt
