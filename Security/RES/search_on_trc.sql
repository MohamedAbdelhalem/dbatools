...
where registry_key = 'HKLM\Software\Microsoft\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQLServer\Parameters'
...
for
select reverse(substring(reverse(ltrim(rtrim(output_text))),1, charindex(' ',reverse(ltrim(rtrim(output_text))))-1)) trc
from @log_table
