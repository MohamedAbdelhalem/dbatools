select replica_id, replica_server_name, [READ_ONLY_ROUTING_LIST],
'ALTER AVAILABILITY GROUP [AG] MODIFY REPLICA ON N'+''''+replica_server_name+''''+' WITH (PRIMARY_ROLE (READ_ONLY_ROUTING_LIST='+[READ_ONLY_ROUTING_LIST]+'));'
from (
...
from (
select replica_id, replica_server_name, routing_priority, 
...
from dbo.availability_read_only_routing_lists_test_balance ro 
inner join dbo.availability_replicas_test ar1
on ro.replica_id = ar1.replica_id
inner join dbo.availability_replicas_test ar2
