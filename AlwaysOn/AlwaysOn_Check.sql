...
else case when connected_state_desc = 'DISCONNECTED' then 'Node rejoin' else 'N/A' end end Need_action, 
group_name, location, 
...
from (
select Server_Name, instance_name,
case 
...
when group_name in ('CRM_AG','AG_Kony','AGCon016','AGCONETL') then  -- 2 DC 1,2 - 3 Nodes (1,2,1)
	case 
...
when group_name in ('testskelta','APG_HAG','RMB_KONY_AG','DH_AG','BAB_NET_AG','ims_abic','iCorp_AG','EFAT_AG','VPPAMDNS_P002') then  -- 2 DC 1,2 - 2 Nodes (1,1) 
	case 
		when substring(server_name, 2,1) = 1 and right(server_name,1) = 1 then 'Node 01' 
...
when group_name in ('KonyAG') then  -- 2 DC 1,2 - 2 Nodes (3,3) 
	case 
		when substring(server_name, 2,1) = 1 and right(server_name,1) = 3 then 'Node 01' 
		when substring(server_name, 2,1) = 2 and right(server_name,1) = 3 then 'Node 02' 
	end
