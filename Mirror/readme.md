...
So the issue was that the transaction log reached the max size of $\colorbox{green}{50 GB}$ plus there were some data waits to sync with the secondary node around 980 KB but because the free space inside the log file was + -1.20 MB all the below solutions failed:
