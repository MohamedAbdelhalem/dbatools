...
[volume] varchar(10), [volume_total_size] varchar(10), [volume_free_size] varchar(10), [volume_used] varchar(10), [threshold] varchar(10), [recommended_extend_size] varchar(10))
end
else
begin
create table server_disk_state__9870(
...
+'background-color: '''+'+'+
case 
when c.name = 'volume_used' then 'case when cast(@volume_used as float) > '+cast(@threshold_pct as varchar)+' then ''red'' else ''green'' end' 
when c.name = 'recommended_extend_size' then 'case when cast(@volume_used as float) > '+cast(@threshold_pct as varchar)+' then ''yellow'' else ''white'' end' 
else '' end+'+'+'''; '+
case when c.name = 'volume_used' then 'color: white' else 'color: black' end +'">'+''''+'+ltrim(rtrim(@'+replace(c.name,' ','_')+'))+'+''''+'</td>'+''''
...
GO
CREATE
Procedure [dbo].[server_disk_state_email] --@project_name = 'Data Hub', @ccteam = '', @with_cc = 0 
(
...
<p>Kindly your response and support is required for <b>'+@project_name+' database server</b>,<p> 
...
USE [msdb]
GO
/****** Object:  Job [capacity_manager_view]    Script Date: 8/14/2022 1:42:35 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 8/14/2022 1:42:35 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [step]    Script Date: 8/14/2022 1:42:35 PM ******/
