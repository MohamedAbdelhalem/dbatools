select 
id, session_id, wait_type, 
format(waiting_tasks_count,'###,###,###') waiting_tasks_count,
wait_time_ms,
cast(cast(wait_time_ms as float) / cast(waiting_tasks_count as float) as numeric(13,4)) avg_wait_time,
signal_wait_time_ms,
cast(cast(signal_wait_time_ms as float) / cast(waiting_tasks_count as float) as numeric(13,4)) avg_signal_time,
resource_wait_time_ms,
cast(cast(resource_wait_time_ms as float)/ cast(waiting_tasks_count as float) as numeric(13,4)) avg_resource_time,
PCT, Running_PCT
from (
select session_id,
row_number() over(order by wait_time_ms desc, session_id) id,
