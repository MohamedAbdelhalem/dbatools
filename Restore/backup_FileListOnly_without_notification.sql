use master
go
declare @backupfile varchar(max) = N'\\npci2.d2fs.albilad.com\DBTEMP\T24DBXTREMIOT3\T24_support\FULL\T24SDC3_HIS_RECID_CONV_2023_08_13.bak'
declare 
@db_name			varchar(1000) = 'T24SDC3_HIS_RECID',
...
union all
select 
'MOVE '+''''+col01+''''+' To '+''''+replace(col02,LEFT(col02,3),'T:\')+''''+','
from @table
union all
select 'NOUNLOAD, '+ 
case when @recovery = 0 then 'NORECOVERY' else 'RECOVERY' end+', '+
