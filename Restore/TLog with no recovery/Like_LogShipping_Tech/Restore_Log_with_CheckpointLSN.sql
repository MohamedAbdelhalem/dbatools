declare @backup_root_path varchar(1000) = '\\npci1.d1fs.albilad.com\DB_Replication_Temp\AccountStatementMigration\D1ENTDBSQPWV4\'
declare @subfolder varchar(1000) = '\FULL'
--declare @backup_root_path varchar(1000) = '\\192.168.2.119\Backups\2024\May\FULL\'
declare @exclude_databases varchar(max) = 'master,model,msdb,ReportServer,ReportServerTempDB'
declare @just_those_databases varchar(max) = 'all'
declare 
...
for
select output_text + case when @subfolder is not null then @subfolder else '' end
from (
select output_text 
from (
...
and (@just_those_databases = 'all' and output_text in (select name from master.sys.databases))
or (@just_those_databases != 'all' and output_text in (select ltrim(rtrim(value)) from master.dbo.separator(@just_those_databases,','))))c
where output_text not in (select ltrim(rtrim(value)) from master.dbo.separator(@exclude_databases,','))
 
 
