use master
go
declare @backupfile varchar(max) = N'\\npci2.d2fs.albilad.com\DB_Replication_Temp\backup_Datahub_13-06-2023_Abdulmohsen\Data_Hub_Cortex_2021-Full Database Backup.bak'
declare 
@db_name			varchar(1000) = 'Data_Hub_Cortex_2021',
...
union all
select 
'MOVE '+''''+col01+''''+' To '+''''+replace(replace(replace(col02,'P:\','O:\'),'P:\','O:\'),'P:\','O:\')+''''+','
from @table
union all
select 'NOUNLOAD, '+ 
case when @recovery = 0 then 'NORECOVERY' else 'RECOVERY' end+', '+
