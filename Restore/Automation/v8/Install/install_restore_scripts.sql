...
CREATE TABLE dbo.auto_restore_job_parameters 
(before_date datetime, db_restore_name varchar(500), username varchar(500), isAG int, [workaround_locations] [varchar](4000),
[modified_date] datetime,
[dba_username] varchar(100))
go

CREATE Trigger dbo.update_modified_date 
on dbo.auto_restore_job_parameters
after insert, update
as
begin
update dbo.auto_restore_job_parameters set modified_date = getdate(), dba_username = suser_sname()
end
go

Insert into dbo.auto_restore_job_parameters (before_date, db_restore_name, username, isAG, workaround_locations)
values ('2023-10-01 02:35:00','T24SDC22','T24SDC22',0,
...
if object_id('[dbo].[backup_files]') is null 
begin
CREATE TABLE [dbo].[backup_files](
	[backup_type] [varchar](10) NULL,
	[backup_time] [datetime] NULL,
	[backup_file_name] [varchar](2000) NULL
)
end
GO
if object_id('[dbo].[table_header]') is null 
begin
CREATE TABLE [dbo].[table_header](
	[col01] [varchar](500) NULL,
	[col02] [varchar](500) NULL,
	[col03] [varchar](500) NULL,
	[col04] [varchar](500) NULL,
	[col05] [varchar](500) NULL,
	[col06] [varchar](5) NULL,
	[col07] [varchar](max) NULL,
	[col08] [varchar](100) NULL,
	[col09] [varchar](500) NULL,
	...
	[BackupTypeDescription] [varchar](max) NULL,
...
from sys.objects
where object_id in (
object_id('[dbo].[show_HeaderOnly]'),
object_id('[dbo].[update_backups_metadata]'),
...
CREATE View [dbo].[show_HeaderOnly]
as
select
col01 backup_file_desc, DatabaseName, CheckpointLSN, DatabaseBackupLSN, BackupStartDate, BackupFinishDate, BackupTypeDescription, backup_file_name
from table_header
go

CREATE --or alter 
PROCEDURE [dbo].[update_backups_metadata](
@before_date				datetime		= '2023-01-12 12:02:00', 
...
@backup_pathes_after_2022	varchar(1000)   = '\\npci2.d2fs.albilad.com\T24_BACKUP_'
)
as
begin
...
declare @backup_files table (backup_type varchar(10), backup_size float, backup_time datetime, backup_file_name varchar(2000))
...
if (select case when @before_date < '2022-12-01' then 0 else 1 end) = 0
begin
...
else if (select case when @before_date >= '2023-01-01' then 0 else 1 end) = 0
begin
	if month(@date_time_f) = month(@date_time_t)
	begin 
		set @xp_folder_full_f_PDC = 'dir cd "'+@backup_pathes_after_2022+@year_f+'\FULL\'+@year_f+'\'+@month_f+'\"'
		set @xp_folder_diff_f_PDC = 'dir cd "'+@backup_pathes_after_2022+@year_f+'\DIFF\'+@year_f+'\'+@month_f+'\"'
		set @xp_folder_logs_f_PDC = 'dir cd "'+@backup_pathes_after_2022+@year_f+'\LOGs\'+@year_f+'\'+@month_f+'\"'
	...
	begin 
		if @year_f = 2022
		begin
			set @xp_folder_full_f_PDC = 'dir cd "'+master.dbo.virtical_array(@locations,';',1)+'"'
			set @xp_folder_full_t_PDC = 'dir cd "'+@backup_pathes_after_2022+@year_t+'\FULL\'+@year_t+'\'+@month_t+'\"'
			set @xp_folder_diff_f_PDC = 'dir cd "'+master.dbo.virtical_array(@locations,';',2)+'"'
			set @xp_folder_diff_t_PDC = 'dir cd "'+@backup_pathes_after_2022+@year_t+'\DIFF\'+@year_t+'\'+@month_t+'\"'
			set @xp_folder_logs_f_PDC = 'dir cd "'+master.dbo.virtical_array(@locations,';',3)+'"'
			set @xp_folder_logs_t_PDC = 'dir cd "'+@backup_pathes_after_2022+@year_t+'\LOGs\'+@year_t+'\'+@month_t+'\"'
...
begin
	...
	if month(@date_time_f) != month(@date_time_t)
	begin
	set @locations = @locations+';'+@xp_folder_full_f_SDC +';'+@xp_folder_full_f_PDC +';'+@xp_folder_diff_f_SDC +';'+@xp_folder_diff_f_PDC +';'+@xp_folder_logs_f_SDC +';'+@xp_folder_logs_f_PDC 
	end
...
(backup_type, backup_size, backup_time, backup_file_name)
select distinct 'full' type, 
replace(ltrim(rtrim(substring(output_text, charindex('M ', output_text)+2, len(output_text) - (charindex('M ', output_text) + charindex(' ',reverse(output_text))+2)))),',',''), 
cast(
...
and output_text not like '%The system cannot find the path specified%'
union all
select distinct 'diff' type, 
replace(ltrim(rtrim(substring(output_text, charindex('M ', output_text)+2, len(output_text) - (charindex('M ', output_text) + charindex(' ',reverse(output_text))+2)))),',',''), 
cast(
convert(varchar(10), cast(ltrim(rtrim(master.dbo.virtical_array(output_text,' ',1))) as datetime),120)+' '+
cast(case when cast(master.dbo.virtical_array(ltrim(rtrim(master.dbo.virtical_array(output_text,' ',3))),':',1) as int) = 12 then 0 else 
...
insert into backup_files
select backup_type, backup_time, backup_file_name from @backup_files where backup_size > 0
except
select backup_type, backup_time, backup_file_name from backup_files
...
from backup_files
where backup_file_name not in (select backup_file_name from table_header where col01 != '*** INCOMPLETE ***')
...
go
declare @team_members varchar(max) = 'Ziad Aqel Abou Eid <ZAbouEid@Bankalbilad.com>'
...
if (select count(*) from sys.databases where name in (select db_restore_name from auto_restore_job_parameters)) > 0
begin
select 
...
if (select count(*) from sys.databases where name in (select db_restore_name from auto_restore_job_parameters)) = 0
begin
--insert into [dbo].[restore_loction_groups] values ('0-J:\SQLSERVER\Data\;1-F:\SQLSERVER\Data\;2-F:\SQLSERVER\Data\;2-L:\SQLSERVER\Data\;2-K:\SQLSERVER\Data\;2-G:\SQLSERVER\Data\;3-H:\SQLSERVER\Data\')

declare @path varchar(1000), @sql varchar(1000)
declare map_c cursor fast_forward
for
select master.dbo.virtical_array(value,'-',2) 
from restore_loction_groups lg cross apply master.dbo.separator(lg.directorys_map,';')
order by id

open map_c
fetch next from map_c into @path
while @@FETCH_STATUS = 0
begin
set @sql = 'xp_cmdshell ''mkdir "'+@path+'"'+''''
print(@sql)
exec (@sql)
fetch next from map_c into @path
end
...
@dear					varchar(500),
@db_mail_profile		varchar(50),
@dba_in_to				varchar(500) = (select dba_username from auto_restore_job_parameters)
...
CREATE
Procedure [dbo].[sp_restore_database_distribution_groups]
(
...
and files.seq_id = loc.location_id
union all
select LogicalName, PhysicalName, Type, loc.loc, files.fileid
from (
...
( 
@before_date				datetime		= '2023-10-01 02:35:00',   --year-month-day
@db_restore_name			varchar(500)	= 'T24SDC22',
@username					varchar(100)	= 'T24SDC22',
...
--case when id = total_files and @before_date between lag(BackupFinishDate,1,1) over(order by BackupFinishDate) and BackupFinishDate  then 1 else 0 end 
case when [with_stopat] = 'default' then 0 else 1 end [recovery]
from (
...
case 
when BackupTypeDescription = 'Transaction Log' and last_file = 1 and @before_date between isnull(LAG(BackupStartDate,1) over(order by BackupStartDate ),0) and BackupStartDate then 'STOPAT = '+''''+convert(varchar(40), @before_date, 120)+''''+'' 
...
begin
	...
	values
	(@db_restore_name, 0, getdate(), @max_id, 1  + @continue_after_file_number, @max_file_name)
...
if (select state_desc from sys.databases where name = @db_restore_name) = 'online'
begin
