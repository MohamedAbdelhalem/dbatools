...
@restore_loction_groups		varchar(1000) = 'default', --'0-T:\SQLSERVER\Data\;1-J:\SQLSERVER\Data\;2-J:\SQLSERVER\Data\;2-K:\SQLSERVER\Data\;2-L:\SQLSERVER\Data\;2-M:\SQLSERVER\Data\;3-N:\SQLSERVER\Data\',
													   ...
													   -- 3 archive file
@option_05					int = 0,
@datafile_type				varchar(100)  = 'default',
@restore_loc_Op5			varchar(500)  = 'default',
@with_recovery				bit = 1,  
...
begin
--fix multi-paths
declare @location_of_non_group table (logicalName varchar(255), PhysicalName varchar(1500),	Type varchar(100), loc varchar(1500), fileid int)
...
if exists (
select count(*), dbo.virtical_array(value, '-', 1)
from master.dbo.Separator(@restore_loction_groups, ';')
where dbo.virtical_array(value, '-', 1) != dbo.virtical_array(@number_of_files_per_type, '-', 1)
group by dbo.virtical_array(value, '-', 1)
having COUNT(*) > 1)

begin

insert into @location_of_non_group
...
--fix multi-paths
select LogicalName, PhysicalName, Type, loc, fileid
from @location_of_non_group
--fix end--------
...
begin
	set @xp_cmdshell = 'xp_cmdshell ''dir cd "'+@restore_loc_Op5+'"'+''''
	insert into @restor_loc_table
	exec (@xp_cmdshell)
end
else if @option_03 = 1
...
select 
LogicalName, 
case when @option_05 = 1 and type = left(@datafile_type,1) then @restore_loc_Op5 else originalPath end originalPath , 
case when PhysicalName like '%.%' then 
		substring(PhysicalName, 1, charindex('.',PhysicalName)-1) else PhysicalName end PhysicalName,
case when PhysicalName like '%.%' then 
		reverse(substring(reverse(PhysicalName), 1, charindex('.',reverse(PhysicalName)))) else 'no_ext' end ext,
...
if (@option_01 + @option_02 + @option_03 + @option_05) = 1
begin
	open dbfiles_cursor
...
if @password = 'default' and (@option_01 + @option_02 + @option_04 + @option_05) = 1 
begin
set @sql = '
RESTORE '+
