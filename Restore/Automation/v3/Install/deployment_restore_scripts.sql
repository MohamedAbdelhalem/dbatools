...
CREATE TABLE dbo.auto_restore_job_parameters (before_date datetime, db_restore_name varchar(500), username varchar(500))
Insert into dbo.auto_restore_job_parameters values ('2022-10-10 11:30:00','T24Prod','T24login')
...
( 
@before_date				datetime = '2022-07-28 11:45:00', 
@db_restore_name			varchar(500) = 'T24SDC10',
@username					varchar(100) = 'T24SDC10',
...
as
begin
declare @final_table table (id int identity(1,1), backup_type varchar(30), backup_time_from datetime, backup_time_to datetime, 
backup_file_name varchar(3000), with_stopat varchar(100), [recovery] tinyint)
...
declare @sql varchar(max), @backupfilename varchar(max), @version int
declare @headeronly table (
BackupName				nvarchar(512),
BackupDescription		nvarchar(255),
BackupType				smallint,
...
EncryptorThumbprint 	varbinary(20)  default NULL,
EncryptorType 			nvarchar(32),
backupfilename			varchar(3000))

declare headeronly_cursor cursor fast_forward
for
...
open headeronly_cursor
fetch next from headeronly_cursor into @backupfilename
while @@FETCH_STATUS = 0
begin

set @sql = 'restore headeronly from disk = '+''''+@backupfilename+''''
if @version = 10
begin
insert into @headeronly (
BackupName,BackupDescription,BackupType,ExpirationDate,Compressed,Position,DeviceType,UserName,ServerName,
...
update @headeronly set backupfilename = @backupfilename where backupfilename is null

end
fetch next from headeronly_cursor into @backupfilename
end
close headeronly_cursor
deallocate headeronly_cursor


insert into @final_table
select BackupTypeDescription, BackupStartDate, BackupFinishDate, backupfilename, 
case 
when BackupTypeDescription = 'Transaction Log' and row_number() over(order by BackupStartDate) = count(*) over() then 'STOPAT = '+''''+convert(varchar(40), @before_date, 120)+''''+'' 
else 'default' end [with_stopat],
case 
when row_number() over(order by BackupStartDate) = count(*) over() then 1 else 0 end [recovery]
from (
select BackupTypeDescription, CheckpointLSN, DatabaseBackupLSN, BackupStartDate, BackupFinishDate, backupfilename
from @headeronly full_
where BackupTypeDescription = 'Database'
union
select BackupTypeDescription, CheckpointLSN, DatabaseBackupLSN, BackupStartDate, BackupFinishDate, backupfilename
from @headeronly diff_
where BackupTypeDescription = 'Database Differential'
and DatabaseBackupLSN in (select CheckpointLSN from @headeronly where BackupTypeDescription = 'Database')
union
select * from (
select top 100 percent BackupTypeDescription, CheckpointLSN, DatabaseBackupLSN, BackupStartDate, BackupFinishDate, backupfilename
from @headeronly tlog_
where BackupTypeDescription = 'Transaction Log'
and DatabaseBackupLSN in (select CheckpointLSN from @headeronly where BackupTypeDescription = 'Database')
order by tlog_.BackupStartDate)a)fin
