use master
go
declare @backupfile varchar(max) = N'D2T24DBSQIWV4_T24PROD_UAT_FULL_20230921_03_02_PM_'
declare @backupfile_forlder varchar(max) = '\\npci2.d2fs.albilad.com\DBTEMP\D2T24DBSQIWV4\T24PROD_UAT\FULL\'
declare 
@db_name			varchar(1000) = 'T24PROD_UAT',
@file_id			varchar(10) = 1,
@recovery			bit = 1,  -- 0 = No Recovery, 1 - Recovery
@with_replace		bit = 1,
@show_size_require	bit = 0,
@option				int = 2,
--@option 1 = database already exists and you will restore it with the same locations.
--@option 2 = use the paths that you have on table restore_location_groups
...
declare @backup_files_folder_table table (output_text varchar(max))
declare 
@xp_backup_folder		varchar(max),
@sql_fileonly			varchar(max),
@sql_headonly			varchar(max),
@composit_backup_files	varchar(max)

set @xp_backup_folder = 'xp_cmdshell ''dir cd "'+@backupfile_forlder+'"'+''''
insert into @backup_files_folder_table
exec (@xp_backup_folder)

select @composit_backup_files = isnull(@composit_backup_files+',','') +'DISK = '+ ''''+@backupfile_forlder + backup_file + ''''+'
'
from (
select substring(output_text, charindex(' ',output_text)+1, len(output_text)) backup_file
from (
select ltrim(rtrim(substring(output_text, charindex('M ',output_text)+1, len(output_text)))) output_text
from @backup_files_folder_table
where output_text like '%M %'
and output_text not like '%<DIR>%'
and output_text like '%'+@backupfile+'%')a)b
order by backup_file

select top 1 @backupfile = backup_file
from (
select substring(output_text, charindex(' ',output_text)+1, len(output_text)) backup_file
from (
select ltrim(rtrim(substring(output_text, charindex('M ',output_text)+1, len(output_text)))) output_text
...
and output_text like '%'+@backupfile+'%')a)b
order by backup_file
--print(@composit_backup_files)

--SELECT @backupfile_forlder+@backupfile

set @sql_fileonly	= 'RESTORE FILELISTONLY FROM DISK = '+''''+@backupfile_forlder+@backupfile+''''
set @sql_headonly	= 'RESTORE HEADERONLY FROM DISK = '+''''+@backupfile_forlder+@backupfile+''''
...
'+syntax_text
from (
select 'RESTORE DATABASE ['+case when @db_name = 'default' then col10 else @db_name end+'] FROM  
'+@composit_backup_files+' 
WITH  FILE = '+col06+',' syntax_text
from @table_header
where col06 = @file_id 
union all
select 
...
exec(@sql)
select * from master.dbo.[error_message](@@spid)
