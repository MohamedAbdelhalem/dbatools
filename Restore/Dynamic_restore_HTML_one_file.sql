USE [master]
GO
/****** Object:  StoredProcedure [dbo].[Dynamic_restore_HTML]    Script Date: 7/3/2022 4:53:21 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
alter procedure [dbo].[Dynamic_restore_HTML_one_file]
...
select 
Command,
cast(round(percent_complete,3) as varchar)+' %'  [File Percent_Complete],
Restore_Type,
...
union all 
select @loop, ','')+''''+'<td style="border:1px solid black; text-align: center; vertical-align: middle;">'+''''+'+ltrim(rtrim(@'+replace(c.name,' ','_')+'))+'+''''+'</td>'+''''
