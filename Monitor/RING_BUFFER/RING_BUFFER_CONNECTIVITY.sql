...
dateadd(hour, 3, convert(datetime,record.value('(Record/ConnectivityTraceRecord/RecordTime)[1]', 'datetime'),120)) as RecordTime,
...
--where recordtype not in ('LoginTimers')
ORDER BY c.id desc
