...
command varchar(1500), command_type varchar(50), sub_command varchar(50), table_name varchar(500), is_table_exist smallint, rows bigint, column_name varchar(500), 
is_column_exist smallint, fn_name varchar(300), is_function_exist smallint, index_name varchar(500), is_index_exist smallint) 

insert into @result --(command,command_type,sub_command,table_name,is_table_exist,rows,column_name,is_column_exist,fn_name,is_function_exist,index_name,is_index_exist) 
select distinct command, command_type, sub_command, 
table_name, 
is_table_exist, 
...
is_index_exist 
from (
select id, command, command_type, sub_command, table_name, column_name,fn_name,index_name,
case when sub_command = 'Add Column' or command_type = 'Create Index' then case when (select count(*) from sys.columns  with (nolock) where name = replace(replace(column_name,']',''),'[','') collate Arabic_100_CI_AS and object_id = object_id(table_name collate Arabic_100_CI_AS)) > 0 then 1 else 0 end 
else '' end is_column_exist,
case when sub_command in ('Add Column') or command_type in ('Create Index','Drop Index') then case when (select count(*) from sys.tables  with (nolock) where object_id = object_id(table_name collate Arabic_100_CI_AS)) > 0 then 1 else 0 end 
...
case when command_type in ('Create Index','Drop Index') then case when (select count(*) from sys.indexes where name = replace(replace(index_name,']',''),'[','') collate Arabic_100_CI_AS and object_id = object_id(table_name collate Arabic_100_CI_AS)) > 0 then 1 else 0 end 
else '' end is_index_exist
from (
select id, command, command_type, sub_command, 
case 
...
case when command_type = 'Alter Table' then case 
when command like '% add %' then 'Add Column'
when command like '% drop %' then 'Drop Column'
end end sub_command,
case 
when command_type = 'Alter Table' then substring(command, charindex(' table ',command)+7, len(command)) 
when command_type = 'Create Table' then substring(command, charindex(' table ',command)+7, len(command)) 
when command_type = 'Drop Table' then substring(command, charindex(' table ',command)+7, len(command)) 
when command_type = 'Create Index' then substring(command, charindex(' on ',command)+4, len(command)) 
when command_type = 'Drop Index' then substring(command, charindex(' on ',command)+4, len(command)) 
end table_name,
case 
when command_type = 'Alter Table' then substring(command, charindex(' as ',command)+4, len(command)) 
end fn_name,
case 
when command_type = 'Alter Table' then substring(command, charindex(' add ',command)+5, len(command)) 
when command_type = 'Alter Table' then substring(command, charindex(' drop ',command)+5, len(command)) 
when command_type = 'create index' then substring(command, charindex('(',command)+1, len(command)) 
end column_name,
case 
when command_type = 'Create Index' then substring(command, charindex(' index ',command)+7, len(command)) 
when command_type = 'Drop Index' then substring(command, charindex(' index ',command)+7, len(command)) 
end index_name
from (
select id, [value] command, case 
...
from master.[dbo].[Separator](@syntax,';')
)a)b
)c
)d
where command is not null

--select count(*), command_type, table_name, column_name
--from @result
--where table_name is not null
--group by command_type, table_name, column_name
--having count(*) > 1

--duplication
--select 
--command,case is_column_exist when 1 then 'alter table '+table_name+' drop column '+column_name else '' end drop_exist_column, command_type,sub_command,table_name,is_table_exist,master.dbo.format(rows,-1) row_numbers,column_name,is_column_exist,fn_name,is_function_exist,index_name,is_index_exist
--from @result
--where table_name = 'dbo.F_BAB_H_E226' order by command_type, column_name

--functions don't exist
--select 
--command,case is_column_exist when 1 then 'alter table '+table_name+' drop column '+column_name else '' end drop_exist_column, command_type,sub_command,table_name,is_table_exist,master.dbo.format(rows,-1) row_numbers,column_name,is_column_exist,fn_name,is_function_exist,index_name,is_index_exist
--from @result
--where command_type ='alter table ' and is_function_exist = 0

--select * from @result where table_name = 'dbo.F_BAB_H_POST_RESTRICT'

select *
from @result
where command is not null
and command_type = 'create index' and is_table_exist = 1 and is_index_exist = 0 

select *
from @result 
where command is not null
and command_type = 'alter table' and is_table_exist = 1 and is_column_exist = 0 

--script generator

--select table_name, column_name, command, rows
--from (
--select table_name, column_name, command, rows,fn_name
--from @result
--where command is not null
--and command_type = 'create index' and is_table_exist = 1 and is_index_exist = 0 
--union all 
--select table_name, column_name, 'GO', rows,fn_name
--from @result
--where command is not null
--and command_type = 'create index' and is_table_exist = 1 and is_index_exist = 0)a
--order by rows desc, table_name, column_name, command
