CREATE PROCEDURE [dbo].[sp_shrink_file](
@file_type				varchar(50) = 'data', --values (data or log).
@file_id				int = 0, -- values (0 = all or file id (1,2,3,4,5,6,...)).
@start_from				int = 0,
@except_files			varchar(100) = '0',
@file_percent			int =	2, -- value = this is the percent number that you want to shrink with with batches like every 5 seconds it will get this value to shrink the file.
@file_used_buffer_mb	int = 2048 -- value = @file_used_buffer_mb MB additional space added on the total used space of the file.
)
as
begin
