...
go
declare 
@username varchar(300) = SUSER_SNAME(),
...
set @password = 'computer password'
end
...
insert into white_list_users (account_number, username, team, is_allowed, email, send_notification)
select account_number, username, team, is_allowed, email, send_notification from @white_list_users
except
select account_number, username, team, is_allowed, email, send_notification from white_list_users
end
else
...
declare @partition_table_style table (volume varchar(10), partition_style varchar(10),vm_physicalLocation varchar(1000),os_deviceId varchar(10), warrning_file_growth int )
...
/****** Object:  Job [capacity_manager_view]    Script Date: 5/4/2023 12:20:15 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 5/4/2023 12:20:15 PM ******/
...
declare @sa_username nvarchar(100)
select @sa_username = name 
from sys.sql_logins
where sid = 0x01
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'capacity_manager_view', 
		...
		@owner_login_name=@sa_username, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [disk_states]    Script Date: 5/4/2023 12:20:15 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'disk_states', 
		...
		@command=N'declare @app_name varchar(1000)
select @app_name = [app_name] from dbo.server_details

if datepart(hour,getdate()) in (7,15)
begin
...
select master.dbo.Win_Version(0)
