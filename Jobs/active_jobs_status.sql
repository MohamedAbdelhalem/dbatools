use master
go
declare @job_name varchar(200), @job_duration varchar(30), @table_rows varchar(30)
select @table_rows =  master.dbo.format(max(rows),-1) 
from sys.partitions 
where object_id in (object_id('dbo.FBNK_FUNDS_TRANSFER#HIS'))
...
select job_name, run_status, message, convert(datetime, rundate+' '+substring(runtime,1,2)+':'+substring(runtime,3,2)+':'+substring(runtime,5,2), 120) run_date_time, duration
from (
select j.name job_name, case run_status 
when 0 then 'Failed'
...
where step_id >= 0
and name = 'DOC_ETL_Read from XML'
)a
