...
case 
	when max(s.freq_type) in (1,4,8,16,32) and max(s.freq_subday_type) in (0,1) then ' at ' + case 
...
when max(s.freq_type) in (4,8,16,32) and max(s.freq_subday_type) >= 1 then ' every ' + cast(max(s.freq_subday_interval) as varchar(20)) +	case max(freq_subday_type)
